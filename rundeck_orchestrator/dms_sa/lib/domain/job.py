...
class Job(object):
  ...
  def setdesc(self,desp):
    self.definition['joblist'][0]['description'][0] = desp

  def setName(self,name):
    self.definition['joblist'][0]['name'][0] = name

  def setNodeFilter(self,nodenames):
    """

    :param name: name:10.74.125.x name:10.x.x.x
    :return:
    """
    names = map(lambda name: "name:%s" % name, nodenames)
    filter = " ".join(names)
    self.definition['joblist'][0]['nodefilters']['filter'][0] = filter
