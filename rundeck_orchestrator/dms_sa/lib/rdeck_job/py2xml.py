...
class Py2XML():
    ...
    def _PyList2XML( self, pyListObj, objName=None ):
        ...
        if objName != None:
          names = objName.split()
          endName = names[0]
        obj_name_map = {"joblist":"job","sequence":"command","configuration":"entry"}
        for childObj in pyListObj:            
            if isinstance( childObj, dict ):
                ...
                childStr += self._PyDict2XML( childObj, obj_name_map.get(endName))
            else:
        ...
        tagStr += "<%(n)s>%(c)s</%(t)s>" % { 'n':objName, 'c':childStr,'t':endName }
        return tagStr
...
def main():
    python_object = {
	'joblist':[{
		"description":["create_vnf"],
                "executionEnabled":['true'],
                "id": ["fd026e9e-8be4-469e-b732-8d9605f7c57e"],
                "name": ["create_vm"],
                "scheduleEnabled": ["true"],
                "sequence keepgoing='false' strategy='node-first'":[{'node-step-plugin':{"copytype":"file","configuration":[{"key":"destinationPath","value":"test"},{"key":"echo","value":"true"},{"key":"sourcePath","value":"sasd"}]}},{"exec":"whoami"}]
